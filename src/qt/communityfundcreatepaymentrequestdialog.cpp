...
void CommunityFundCreatePaymentRequestDialog::click_pushButtonSubmitPaymentRequest()
{
    ...
    if(this->validate())
    {
        ...
        if (nReqAmount <= 0 || nReqAmount > proposal.GetAvailable(*pcoinsTip, true)) {
            QMessageBox msgBox(this);
            QString str = tr("Requested amount must be greater than 0 XSK (Zero)\n");
            if (nReqAmount > proposal.GetAvailable(*pcoinsTip, true)) {
                str = tr("Requested amount %1 is more than avaible coins in the proposal (%2)\n")
                    .arg(
                        StakeWorkUnits::formatWithUnit(StakeWorkUnits::XSK, nReqAmount),
                        StakeWorkUnits::formatWithUnit(StakeWorkUnits::XSK, proposal.GetAvailable(*pcoinsTip, true))
                    );
            }
            msgBox.setText(str);
            msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
        ...
        if (curBalance <= 10000) {
            ...
            std::string str = "You require at least " + fee + " XSK mature and available to create a payment request\n";
            msgBox.setText(tr(str.c_str()));
            msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
            msgBox.setIcon(QMessageBox::Warning);
            msgBox.setWindowTitle("Insufficient XSK");
            msgBox.exec();
            return;
        }
