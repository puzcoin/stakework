...
void FreespaceChecker::check()
{
    ...
    try {
        ...
        if(fs::exists(dataDir))
        {
            ...
            {
                replyStatus = ST_OK;
                replyMessage = tr("StakeWork will try to import an old wallet.dat file.");
            } else {
...
void Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/stakework"));
...
void Intro::makeDefaultConfF()
{
    ...
    {
        char t[] = R"(##
## stakework.conf configuration file. Lines beginning with # are comments.
##

# Network-related settings:

# Run on the test network instead of the real stakework network.
#testnet=0
...
#
# JSON-RPC options (for controlling a running StakeWork/stakeworkd process)
#

# server=1 tells StakeWork-Qt and stakeworkd to accept JSON-RPC commands
#server=0
...
# server=1 tells StakeWork-Qt to accept JSON-RPC commands.
# it is also read by stakeworkd to determine if RPC should be enabled
