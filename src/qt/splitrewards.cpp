    ...
    comboAddress(new QComboBox)
{
    ...
    {
        comboAddress->insertItem(i++, it.first +" ("+ QString::fromStdString(FormatMoney(it.second)) +" XSK)",it.first);
    }
...
void SplitRewardsDialog::showFor(QString sin)
{
    ...
    strDesc->setText(tr("For each block, %1 XSK will go to the Community Fund, %2 and %3 will be accumulated in your own address").arg(QString::fromStdString(FormatMoney(nCFundContribution)), descs.join(", "), QString::fromStdString(FormatMoney(PercentageToNav(availableAmount)))));
