...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            ...
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "XSK is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CStakeWorkAddress destaddress(Address);
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 XSK\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CStakeWorkAddress address(proposal.Address);
    ...
    std::string Secret = sRandom + "I kindly ask to withdraw " +
            std::to_string(nReqAmount) + "XSK from the proposal " +
    ...
    if (fAllAccounts || (account == strAccount))
    {
        ...
        entry.pushKV("canStake", (::IsMine(*pwalletMain, r.destination) & ISMINE_STAKABLE ||
                                          (::IsMine(*pwalletMain, r.destination) & ISMINE_SPENDABLE &&
                                           !CStakeWorkAddress(r.destination).IsColdStakingAddress(Params()))) ? true : false);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; StakeWork server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listproposals(const UniValue& params, bool fHelp)
{
    ...
    if(pcoinsTip->GetAllProposals(mapProposals))
    {
        for (CProposalMap::iterator it = mapProposals.begin(); it != mapProposals.end(); it++)
        {
            ...
            if (showMine)
            {
                CTxDestination address(CStakeWorkAddress(proposal.Address).Get());
                isminefilter mine = IsMine(*pwalletMain, address);
